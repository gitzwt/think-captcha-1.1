...
/**
 * tp5.0 Captcha1.0修改版,新增设置type驱动 支持cache存储
 * Class Captcha
...
class Captcha
{
    ...
    public function check($code, $id = '')
    {
        ...
        // 验证码不能为空
        $secode = $this->type == 'session' ? Session::get($key, '') : Cache::get($key);
        ...
        if ($this->authcode(strtoupper($code)) == ($this->type == 'session' ? $secode['verify_code'] : $secode)) {
            $this->reset && ($this->type == 'session' ? Session::delete($key, '') : Cache::rm($key));
            return true;
        }
     ...
     * 验证码保存到cache的格式为: Cache:set('验证码值','验证码过期时间');
     * @access public
    ...
    public function entry($id = '')
    {
        ...
        if ($this->type == 'session') {
            ...
            Cache::set($key . $id, $code, $this->expire);   // 把校验码保存到cache
        }
